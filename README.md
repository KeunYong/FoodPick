...
| 동네 맛집 | 모드를 바꾸면 집 근처 가게를 룰렛으로 뽑습니다. 가게는 페이지에서 등록합니다. |
...
assets/js/places.js    항상 보일 맛집을 코드로 박는 곳 (보통은 비워둡니다)
...
페이지에서 직접 등록합니다. `동네 맛집` 모드 → `내 맛집 관리` 를 펼치면
이름·업종·도보 시간·가격대·태그·지도 링크·메모를 넣을 수 있고, 등록한
가게는 수정과 삭제도 됩니다. 이름만 필수입니다.

등록한 목록은 **그 브라우저에만** 저장됩니다(localStorage). 폰으로 옮기거나
백업하려면 관리 패널 아래 `백업 · 옮기기` 에서 `현재 목록 복사` 로 JSON 을
받아두고, 다른 기기에서 붙여넣은 뒤 `붙여넣은 내용 불러오기` 를 누르면 됩니다.

누구 브라우저에서 열어도 항상 보이게 하려면 `assets/js/places.js` 에
코드로 박아둘 수도 있습니다. 그렇게 넣은 항목은 페이지에서 수정·삭제되지
않고 목록에 `코드` 로 표시됩니다.
